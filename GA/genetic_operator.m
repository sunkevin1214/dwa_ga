function f  = genetic_operator(parent_chromosome, V, mu, mum, l_limit, u_limit, benchmark, crossover_rate, mutation_rate, weight)
...
for i = 1:2:N
    ...
    if rnd<=crossover_rate
        for j = 1:V
            ...
            par1 = parent_chromosome(i, j);
            par2 = parent_chromosome(i+1, j);
            yl = l_limit(j);
            yu = u_limit(j);
            rnd = rand();
            ...
            parent_chromosome(i, j) = chld1;
            parent_chromosome(i+1, j) = chld2;
        end
        child(i, 1:V) = parent_chromosome(i, 1:V);
        child(i+1, 1:V) = parent_chromosome(i+1, 1:V);
        child(i,V + 1) = feval(['weight_', benchmark],child(i, 1:V), weight);
        child(i+1,V + 1) = feval(['weight_', benchmark],child(i+1, 1:V), weight);
    else
        child(i, 1:V+1) = parent_chromosome(i, 1:V+1);
        child(i+1, 1:V+1) = parent_chromosome(i+1, 1:V+1);
    end
end
...
for j = 1:N
    ...
    child(j,V + 1) = feval(['weight_', benchmark], child(j, 1:V), weight);
end
